...
impl SitesApi {
    ...
    async fn list(
        ...
        let sql = format!("SELECT {COLS} FROM knievel.sites ORDER BY id DESC LIMIT 500");
        match sqlx::query_as::<_, Site>(&sql).fetch_all(&mut *tx).await {
            Ok(items) => ListResp::Ok(Json(SiteList {
                items,
                next_cursor: None,
            })),
            Err(e) => {
