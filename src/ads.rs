...
impl AdsApi {
    ...
    async fn list(
        ...
        let sql = match resolved.after_id {
            None => format!("SELECT {COLS} FROM knievel.ads ORDER BY id DESC LIMIT $1"),
            Some(_) => {
                format!("SELECT {COLS} FROM knievel.ads WHERE id < $2 ORDER BY id DESC LIMIT $1")
            }
        };
        let q = sqlx::query_as::<_, Ad>(&sql).bind(resolved.bumped_limit);
